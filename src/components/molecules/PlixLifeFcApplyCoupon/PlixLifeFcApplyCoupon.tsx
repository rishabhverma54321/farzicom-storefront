...
import { getDBIdFromGraphqlId, getGclid } from "@src/core/utils";
    ...
    return (
      <>
        ...
        {promoCodeErrors.length > 0 &&
          !promoCode &&
          promoCodeErrors.map((err, index) => {
            if (err.field === "promoCode") {
              return <S.ErrorMessage key={`${index}`}>Invalid Coupon code.</S.ErrorMessage>;
            }
            return <S.ErrorMessage key={`${index}`}>{err.message}.</S.ErrorMessage>;
          })}
        {promoCode && (
