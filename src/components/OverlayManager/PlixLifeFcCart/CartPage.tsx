...
import { TypedCouponPrepaidDiscount } from "@components/molecules/PlixLifeFcApplyCoupon/queris";
import {
    ...
    return (
      <>
        ...
        {itemsInCart?.itemsToShow?.map(({ id, variant, quantity, totalPrice, productType, newQuantity }, index) => {
          return (
            <CartRow
              key={id ? `id-${id}-${index}-${Math.random()}` : `idx-${index}-${Math.random()}`}
              index={index}
              id={variant?.product?.id || ""}
              name={variant?.product?.name || ""}
              ...
              discountedItemsBXGY={discountedItemsBXGY?.length ? true : false}
              onRemove={(
                setLoading,
                upsell_id,
                items,
