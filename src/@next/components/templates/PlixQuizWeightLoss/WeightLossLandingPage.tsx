...
export interface IWeightLossLandingPage {
  content?: {
    metadata?: Array<any>;
  };
}

export const WeightLossLandingPage: React.FC<IWeightLossLandingPage> = ({
  content,
}) => {
  ...
  const handleQuizEvents = (redirect: string) => {
    if (gtmConfig?.plixQuizWeightLoss?.enable) {
      (window.dataLayer = window.dataLayer || []).push({
        event: gtmConfig?.plixQuizWeightLoss?.value,
        ecommerce: {
  ...
  return (
    <div className={style.container}>
      ...
      {weightCause?.enable ? <QuizCauseSection data={weightCause} handleQuizEvents={handleQuizEvents} /> : <></>}
      {weightLoss?.enable ? <WeightLossSection data={weightLoss} handleQuizEvents={handleQuizEvents} /> : <></>}
      {goalSection?.enable ? <GoalSection data={goalSection} /> : <></>}
      {resultSection?.enable ? <ResultSection data={resultSection} handleQuizEvents={handleQuizEvents} /> : <></>}
      {reviewSection?.enable ? <ReviewSection data={reviewSection} googleRating={googleRating} /> : <></>}
      {
        ingredientSection?.enable ? (
          <IngredientSection data={ingredientSection} />
        ) : (
          <></>
        )
      }
      {
        relatedProducts?.enable ? (
          <RelatedProductSection data={relatedProducts} fastResultProducts={fastResultProducts} popupstateHandler={popupstateHandler} />
        ) : (
          <></>
        )
      }
      ...
      <PlantATree />
      {stickyBottom?.enable ? <StickyBottom data={stickyBottom} handleQuizEvents={handleQuizEvents} /> : <></>}
    </div >
  );
};
WeightLossLandingPage.displayName = "WeightLossLandingPage";
