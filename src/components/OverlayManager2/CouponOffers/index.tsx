...
import MemoTimes from "@components/atoms/SvgIcons/Times";
import MemoBadgePercent from "@components/atoms/SvgIcons/BadgePercent";
...
const handlePromoCodeApply = async promoCode => {
  ...
  if (errors) {
    messageUpdate("Failed to apply coupon code.","error") 
  } else if (!voucherCode) {
    messageUpdate("Invalid Coupon Code","error") 
  } else {
    messageUpdate("Coupon Code Applied","success") 
  }
};
          ...
          <div className="co__available__body">
            {
              Array.isArray(available_offers) && available_offers.map((item, index) => {
                ...
                disableApplyButton || typeof isValid === "boolean"|| typeof isValid === "undefined"
                  ? !isValid
                  : false;
                const isAppliedCode = promoCodeDiscount?.voucherCode != null ? promoCodeDiscount?.voucherCode : ""
                return (
                    ...
                    <div className="new-offer-coupon">
                      ...
                      <div className="new-offer-content">
                        <div className="">
                          ...
                          <span className={`new-coupon-copy theme-text-color`} onClick={(e) => {disableCoupon ? (e.preventDefault()) : handlePromoCodeApply(item?.code)}}>{isAppliedCode == code ? "Applied" : "Apply"}</span>
                        </div>
