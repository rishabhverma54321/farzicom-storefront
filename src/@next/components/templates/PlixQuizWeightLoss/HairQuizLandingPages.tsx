...
import QuizCauseSection, {
  IWeightGainInfo,
} from "./components/QuizCauseSection";
import WeightLossSection, {
  IWeightLossInfo,
} from "./components/WeightLossSection";
...
import IngredientSection, {
  IngredientSectionInfo,
} from "./components/IngredientSection";
import FaqSection, { faqDataInfo } from "./components/FaqSection";
import StickyBottom from "./components/StickyBottom";
import RelatedProductSection, {
  IRelatedProductsInfo,
} from "./components/RelatedProductSection";
import HairSolutionSection, {
  IHairSolutionSectionProps,
} from "./components/HairSolutionSection";

export interface IHairQuizLandingPage {
  content?: {
    metadata?: Array<any>;
  };
  isSkin?: boolean;
...
const HairQuizLandingPages: React.FC<IHairQuizLandingPage> = ({
  content,
  isSkin,
  pageConfig,
}) => {
  ...
  const hairGrowthBanner: ICollageBanner =
    metadata &&
    getMetadataValue(metadata, "hair_growth_banner") &&
    parseJson(getMetadataValue(metadata, "hair_growth_banner"));
  ...
  const hairSolution: IHairSolutionSectionProps =
    getMetadataValue(metadata, "hair_solution_section") &&
    parseJson(getMetadataValue(metadata, "hair_solution_section"));
  ...
  const handleQuizEvents = (redirect: string) => {
    if (
      gtmConfig?.plixQuizHairMaleLandingPage?.enable ||
      gtmConfig?.plixQuizHairFemaleLandingPage?.enable
    ) {
      (window.dataLayer = window.dataLayer || []).push({
        event:
          pageConfig?.quiz === "male"
            ? gtmConfig?.plixQuizHairMaleLandingPage?.value
            : gtmConfig?.plixQuizHairFemaleLandingPage?.value,
        ecommerce: {
  ...
  return (
    <div className={`${style.container} ${style.hariQuiz}`}>
      {popupstate && (
      ...
      {bannerSection?.enable ? (
        <BannerSection
          ...
          handleQuizEvents={handleQuizEvents}
        />
      ) : (
        <></>
      )}
      ...
      {beautyBannerInfo?.enable ? (
        <BeautyBanner data={beautyBannerInfo} />
      ) : (
        <></>
      )}
      {hairSolution?.enable ? (
      ...
      {hairGrowthBanner?.enable ? (
        <div className={style.hariQuiz_growth}>
          <CollageBanner pageConfig={pageConfig} data={hairGrowthBanner} />
        </div>
      ) : (
        <></>
      )}
      {collageBannerInfo?.enable ? (
        <CollageBanner pageConfig={pageConfig} data={collageBannerInfo} />
      ) : (
        <></>
      )}
      {goalSection?.enable ? (
        <GoalSection data={goalSection} pageConfig={pageConfig} />
      ) : (
        <></>
      )}
      {resultSection?.enable ? (
        <ResultSection
          data={resultSection}
          handleQuizEvents={handleQuizEvents}
          isSkin={isSkin}
      ...
      {relatedProducts?.enable ? (
        <div
          className={
            pageConfig?.headingColor ? style.hariQuiz_relatedproduct : ""
          }
        >
      ...
      <FaqSection data={faqData} pageConfig={pageConfig} />
      <PlantATree />
      {stickyBottom?.enable ? (
        <StickyBottom
          data={stickyBottom}
...
export default HairQuizLandingPages;
