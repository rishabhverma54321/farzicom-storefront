...
export const NewAddToCartButton: React.FC<INewAddToCartButton> = ({
  ...
  const findVariantQuantity: any =
    (items && items?.find((item) => item?.variant?.id === variantId))
      ?.quantity || 0;
  ...
  const productConfig =
    getMetadataValue(product?.metadata, "product_config") &&
    parseJson(getMetadataValue(product?.metadata, "product_config"));
  ...
  const handleAfterATC = (
    ...
    if (product && product?.category?.slug === "others-also-liked-products") {
  ...
  const handleClick = () => {
    const maxCapping = productConfig?.product_capping || null;
    ...
    if (disabled) {
      ...
      // messageUpdate(`Max ${maxCapping} same products can be added`, "error");
    } else {
      handleAddToCart();
    }
  };
        ...
        >
          ...
          <S.MainText className={`${buttonClassName}__mainText`}>
            {disabled && !itemAdded && !loading ? (
              <FormattedMessage id="NM" defaultMessage="Notify Me" />
            ) : (
              <>
                {loading ? (
                  <>
