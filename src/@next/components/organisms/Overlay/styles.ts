...
export const Lightbox = styled.div<IStyleProps>`
  display: flex;
  position: relative;
  width: ${({ unsetposition }) => (unsetposition ? "100% !important" : "500px")};
...
export const Overlay = styled.div<IStyleProps>`
  display: flex;
  /* position: fixed; */
  position: ${({ unsetposition }) => (unsetposition ? "unset" : "fixed")};

  overflow-y: auto;
  width: 100%;
  height: 100%;
  min-height: ${({ unsetposition }) => (unsetposition ? "unset" : "100vh")};
  top: 0;
  z-index: 4;
