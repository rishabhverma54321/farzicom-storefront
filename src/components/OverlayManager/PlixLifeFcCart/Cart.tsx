...
import * as S from "./s";
import Page from "@temp/themes/plixlifefc/views/Product/Page";
...
import BxGyProgressbar from "./BxGyProgressbar";
import MemoArtboard from "@components/atoms/SvgIcons/MemoArtboard";
import MemoCartRightArrow from "@components/atoms/SvgIcons/MemoCartRightArrow";
...
// For B2G2 offers
const offer_progressbar_data_bx_gy =
getMetadataValue(ShopMetaContextValue, "bx_gy_offer_progress_bar") &&
parseJson(getMetadataValue(ShopMetaContextValue, "bx_gy_offer_progress_bar"));
  // for car proceed logo
  const cartLogo = getMetadataValue(ShopMetaContextValue, "proceed_pay_logo") &&
  parseJson(getMetadataValue(ShopMetaContextValue, "proceed_pay_logo"));
  return (
        ...
        <div className="cart-plix">
          ...
          {items?.length || checkoutLoading ? (
            <>
              ...
              >
                <>
                 <BxGyProgressbar checkoutLoading={checkoutLoading} isBoxInCart={isBoxInCart} handlePromoCodeApply={handlePromoCodeApply} items={items} overlay={overlay} />
                ...
                <div className="cart-plix__sectionHeader">
                  <span>Product Summary</span>
                  <button>💰Total Savings: ₹{paymentSummary?.couponDiscount?.gross?.amount}</button>
                </div>
                  ...
                  >
                    ...
                    >
                      {checkoutLoading ? (
                        ...
                        {/* <span>Proceed to Pay</span> */}
                        {cartLogo?.enable ? (<>
                          <div className="new-cart-proceed-pay">
                          <div className="cart-proceed-text">
                            <span>Place Order</span>
                            {cartLogo?.extraOffText ? <span>{cartLogo?.extraOffText}</span> : <></>}
                          </div>
                          <div className="new-cart-proceed-right">
                            <div className="new-cart-pay-logo">
                            {Array.isArray(cartLogo?.cartLogo) && cartLogo?.cartLogo.length ? (
                              cartLogo?.cartLogo.map((item, i)=>{
                                return(
                                <div key={`cartLogo_${i}`}>
                                  <img src={item} />
                                </div>
