...
import MyCustomLink from "@src/components/atoms/MyCustomLink";
import {
  useAuthState,
  useCart,
  useCartState,
...
import { ENABLE_GA4, META_DEFAULTS } from "Themes/config";
...
const CartRow: React.FC<IProps> = ({
  ...
  const productConfig =
    getMetadataValue(variant?.product?.metadata, "product_config") &&
    parseJson(getMetadataValue(variant?.product?.metadata, "product_config"));
      ...
      onClick={e => {
        if ((!e.detail || e.detail == 1) && !loading) {
          ...
          if (
            ...
            messageUpdate(
              `Max ${maxCapping} same products can be added`,
              "error"
            );
            return;
          }
