...
export interface IAddToCartSectionLotus {
  itemAdded: boolean;
...
const AddToCartSectionLotus: React.FC<IAddToCartSectionLotus> = ({
    ...
    try {
      add(variantId, quantity).then(res => {
        ...
        if (res.data) {
          ...
          if (CLIENT === clients.LOTUS_NEW || CLIENT === clients.LOTUS_STAGE)
  ...
  if (CLIENT === "lotus-new" || CLIENT === "lotus-stage") {
    return (
      <>
        ...
        <S.OfferBox>
          <S.OfferHeader>OFFER:</S.OfferHeader>
          <S.OfferList>
            {productOffers?.map(offer => {
              if (offer) return <S.Offer key={offer}>{offer}</S.Offer>;
            })}
          </S.OfferList>
        ...
        <S.StickyButtons className={`${stickyButtonClassVisible}`}>
        ...
        <S.DetailsBox>
          <S.DetailBox>
            <S.DetailsImg src="https://lotus-media.farziengineer.co/hosted/MII-Green-8c2f5e0840d4.jpg" />
            <S.DetailsLabel>Made in india</S.DetailsLabel>
          </S.DetailBox>
          <S.DetailBox>
            <S.DetailsImg src="https://lotus-media.farziengineer.co/hosted/BB-Green-9df1aab07b1e.jpg" />
            <S.DetailsLabel>Best before</S.DetailsLabel>
          </S.DetailBox>
        </S.DetailsBox>
            ...
            <TypedCheckForDelivery variables={{ pincode: pinCode }}>
              {({ data }) => {
                ...
                return data?.pincode === null ? (
                  <S.CheckResult>Not Avalible</S.CheckResult>
                ) : (
                  <S.CheckResult>
                    Avalible at {data?.pincode.city}
                  </S.CheckResult>
                );
              }}
            </TypedCheckForDelivery>
  ...
  return (
    <S.Container className={className}>
      ...
      {/* <S.StickyButtons
        className={` ${stickyButtonClassVisible} lessi ${className}__productName`}
      >
        {product.category.slug !== "free-gift-products" && (
          <>
            <StyledAddToCartButton
...
AddToCartSectionLotus.displayName = "AddToCartSectionLotus";
export default AddToCartSectionLotus;
