...
export const ProductCardWizzy: React.FC<IProductCardPlixlifeProps> = ({
  product,
  hoverBg,
  isWishlist,
  from,
  ...
  // const metadata = null;
  const metaObject = parseJson(
    product?.attributes?.find(attr => attr?.name === "product_metadata")
      ?.values[0]?.value[0]
  );
  const metadata = Object.keys(metaObject)?.map(key => ({
    key,
    value: metaObject[key],
  }));

  let defaultVariant = parseJson(
    product?.attributes?.find(attr => attr?.name === "default_variant")
      ?.values[0]?.value[0]
  );
  defaultVariant = {
    ...defaultVariant,
    metadata: Object.keys(defaultVariant?.metadata)?.map(key => ({
      key,
      value: defaultVariant?.metadata[key],
    })),
  };

  // console.log('productcardProduct', product,metaObject, defaultVariant )
  ...
  const cost_price = parseInt(defaultVariant?.cost_price);
  const discounted_price = parseInt(defaultVariant?.price);

  // const defaultVariantAttributes = product?.attri
  ...
  const disableButton = !useCanAddToCart(inStock, variantId, variantStock, 1);
  ...
  const productCardTag =
    productCardAttributes?.card_tag || product?.categories[0]?.name;
      ...
      <>
        ...
        >
          <S.CardTopHeader>
            ...
            <S.Rating>
              <MyRating
                rating={getRoundedRating(averageRating || 5)}
                isReadOnly
              />
            </S.Rating>
          </S.CardTopHeader>
...
ProductCardWizzy.displayName = "ProductCardWizzy";
export default React.memo(ProductCardWizzy, mmatchPropsAreEqual);
